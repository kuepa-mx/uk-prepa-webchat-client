...
export const MessageInput = () => {
  ...
  const { conversation, attachedFiles, fileAttachmentConfig, preEngagementData } = useSelector((state: AppState) => ({
  ...
  const send = async () => {
    ...
    if (!preEngagementData) {
      log.error("Failed sending message: No data obtained in PreEngagement Phase");
    }
    setIsSending(true);

    let preparedMessage = conversation.prepareMessage();
    preparedMessage.setAttributes({ email: preEngagementData?.email, requestType: preEngagementData?.requestType });
      ...
      <InputBox element="MESSAGE_INPUT_BOX" disabled={isSending}>
        <Box as="div" {...innerInputStyles}>
          ...
          <Box {...messageOptionContainerStyles}>
            ...
            >
              <SendIcon decorative={false} title="Enviar" size="sizeIcon30" />
            </Button>
          </Box>
        </Box>
        {attachedFiles && (
